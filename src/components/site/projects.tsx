    ...
    <section id="projects" className="section-pad bg-surface">
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
        ...
        <Reveal className="mb-8 flex flex-wrap gap-2">
          {[{ id: "all", label: t("projects.all") }, ...categories.map((c) => ({ id: c, label: c }))].map(
            (c) => (
              <button
                key={c.id}
                type="button"
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="space-y-5 text-sm leading-relaxed">
          <Block title={t("projects.overview")}>
            {project.full_description || project.short_description || t("projects.tbd")}
          </Block>
          <Block title={t("projects.problem")}>{t("projects.tbd")}</Block>
          <Block title={t("projects.solution")}>{t("projects.tbd")}</Block>
          <Block title={t("projects.features")}>{t("projects.tbd")}</Block>
